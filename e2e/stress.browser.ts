...
for (const selectors of tracing ? ['global', 'scoped'] : ['scoped'])
  test(`500 mounted cards stay responsive under streaming, delayed saves and conflicts (${selectors} selectors)`, async ({
    page,
    browserName,
    browser,
  }, info) => {
    ...
    const finishTrace = tracing ? await startTrace(page, info) : undefined;
    try {
      ...
      await expect
        .poll(() =>
          page.evaluate(() =>
            window.stress.server.state.runs.some((r) => r.status === 'cancelled'),
          ),
        )
        .toBe(true);
      expect(
      ...
      if (selectors === 'global') await expect(conflict).toBeVisible();
      else await conflict.waitFor({ state: 'visible' });
