...
test.beforeEach(async ({ page }) => {
  ...
  await page.locator('.canvas-host').evaluate((el) => {
    el.addEventListener('gotpointercapture', (event) => {
      (el as HTMLElement).dataset.capturedPointer = String((event as PointerEvent).pointerId);
    });
  });
});
  ...
  test(`marquee cancellation via ${reason} restores selection and allows the next drag`, async ({
    ...
    if (reason === 'Escape') await page.keyboard.press('Escape');
      ...
      await page.locator('.canvas-host').evaluate((el, reason) => {
        ...
        const pointerId = Number((el as HTMLElement).dataset.capturedPointer);
        if (reason === 'lostcapture') el.releasePointerCapture(pointerId);
        else
          el.dispatchEvent(
    ...
    await page.mouse.up();
    await expect(page.locator('.selection-rectangle')).toHaveCount(0);
    await expect(node(page, 'pb')).toHaveClass(/selected/);
    await expect(node(page, 'pa')).not.toHaveClass(/selected/);
...
for (const delta of [1, 4, 7]) {
  test(`a header click with ${delta}px jitter selects without moving or saving`, async ({
    ...
    const grip = (await node(page, 'pa').locator('.card-grip').boundingBox())!;
    const before = await node(page, 'pa').boundingBox();
    await page.mouse.move(grip.x + 50, grip.y + 15);
    await page.mouse.down();
    await page.mouse.move(grip.x + 50 + delta, grip.y + 15, { steps: delta });
    await page.mouse.up();
    await expect(node(page, 'pa')).toHaveClass(/selected/);
    expect((await node(page, 'pa').boundingBox())!.x).toBe(before!.x);
    expect(requests).toEqual([]);
  });
}
for (const gesture of ['move', 'resize'] as const) {
  for (const reason of ['Escape', 'tool', 'blur', 'pointercancel'] as const) {
    test(`${reason} cancels card ${gesture} without a remount or a save`, async ({ page }) => {
      ...
      const card = node(page, 'pa');
      await card.locator('.card-grip').click();
      const before = (await card.boundingBox())!;
      await card.evaluate((el) => {
        (el as HTMLElement).dataset.survives = 'yes';
      });
      const handle =
        gesture === 'move' ? card.locator('.card-grip') : card.locator('[data-resize="se"]');
      const bounds = (await handle.boundingBox())!;
      await page.mouse.move(
        bounds.x + Math.min(50, bounds.width / 2),
      ...
      if (reason === 'Escape') await page.keyboard.press('Escape');
        ...
        await page.locator('.canvas-host').evaluate((el) =>
          el.dispatchEvent(
            new PointerEvent('pointercancel', {
              pointerId: Number((el as HTMLElement).dataset.capturedPointer),
              bubbles: true,
            }),
          ),
        );
      await page.mouse.up();
      await expect(card).toHaveAttribute('data-survives', 'yes');
      await expect.poll(async () => (await card.boundingBox())!.x).toBeCloseTo(before.x);
      expect((await card.boundingBox())!.width).toBeCloseTo(before.width);
      expect(requests).toEqual([]);
    });
  }
}
