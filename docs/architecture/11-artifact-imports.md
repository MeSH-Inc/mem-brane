...
A PDF records filename, original asset identity/hash, page count and a pdf-text-v1 representation. Ready representations contain complete, ordered page text and an explicit extractor version. Every page retains its number, including empty pages. The local extraction budget is 100 pages, 20,000 characters and a conservative 24,000-byte serialized-text bound. Exceeding a page/text bound stores the original with an unavailable representation; partial extraction is never silently submitted. Graphical PDFs without embedded text are retained with an explicit needs-OCR reason. Malformed, locked or resource-exhausting files fail import and retain their local retry material.
...
Run inputs freeze the full representation and extractor version in immutable revisions. Execution reads those pages, never current extraction state, and verifies the original asset hash. Images use the same original-byte integrity path. Enhanced OCR has a separate accepted model/cost policy and durable result. Explicit adoption chooses a new immutable representation for one PDF block without rewriting submitted representations. Enhanced output has its own 100-page/20,000-characters-per-page and 10 MiB evidence bounds; expanded model context still passes the existing context admission limits. See [OCR admission](../ocr-admission.md).
