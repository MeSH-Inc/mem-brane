...
Run has immutable submission identity, frozen inputs, provider/model/options, continuation point and optional retry-of ID. Lifecycle, lease, error, usage and finish time are operational mutable fields. A frozen context manifest owns ordered local source/reference/prompt entries and a parent conversation point. RunInput is its expanded read projection, including lineage. Conversation messages identify their originating run, and retries share the same manifest. See 12-context-manifests.md. RunOutput binds final immutable revision to Run. RunCheckpoint is mutable partial text, replaced in batches. RunAttempt records invocation boundary, heartbeat and outcome; a manual retry is a separate Run with a new attempt.
...
Import operations bind delivery identity to bytes and destination intent. PDF and image originals remain immutable assets; model representation readiness is distinct from storage readiness. See [artifact imports](11-artifact-imports.md).
